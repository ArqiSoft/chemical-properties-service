...
COPY . /usr/src/sds-chemical-properties-service
WORKDIR /usr/src/sds-chemical-properties-service
RUN mvn -Pdocker package

FROM openjdk:10.0.2-jre-slim-sid
VOLUME /logs
RUN mkdir /temps
ENV SPRING_BOOT_APP sds-chemical-properties-service.jar
...
COPY --from=builder /usr/src/sds-chemical-properties-service/target/$SPRING_BOOT_APP ./
